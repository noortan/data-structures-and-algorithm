...
const screenForNames = (arr) => {
  ...
  let newArr = [];
  arr.map(el => {
    if(/^(Mr|Mrs|Ms|Dr|Er)\..[A-Za-z]+$/.test(el) === true) {
      newArr.push(el);
    }
  });
  return newArr;
  ...
  for( i = 0; i < board.length; i++) {
    for(col = 0; col  < board[i].length; col++) {
      if(board[0][col] === 'X' && board[1][col] === 'X' && board[2][col] === 'X') {
        return(true);
      } else if(board[0][col] === 'O' && board[1][col+1] === 'O' && board[2][col+2] === 'O'){
        return (true);
      } else if(board[i][col] === 'X' && board[i][col+1] === 'O' && board[i][col+2] === 'X'){
        return(false);
      } else if(board[i][col] === '' && board[i][col+1] === '' && board[i][col+2] === ''){
        return(false);
      }
    }
  }
