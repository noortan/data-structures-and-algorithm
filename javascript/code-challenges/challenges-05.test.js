...
const toLastNames = people => {
  // Solution code here...
  let namesArray = people.map(el => `${el.firstName} ${el.lastName}`);
...
const addValues = (arr) => {
  // Solution code here...
  let valuesAdd = arr.reduce((acc, el) => {
    acc = acc + el;
    return acc;
  }, 0) ;
  return valuesAdd;
...
const returnNames = (arr) => {
  // Solution code here...
  let namesArray=arr.reduce((acc,val,indx )=>{
    acc[indx]=val.name ;
    return acc;
  },[]) ;
  return namesArray;
...
const reversedString = (str) => {
  ...
  let reverseOrder = newStr.reduce((acc, val) => val+ acc, '');
  return reverseOrder;
  // Solution code here...
};
