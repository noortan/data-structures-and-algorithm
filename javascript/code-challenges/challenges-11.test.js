...
function transformToLis(obj){
  ...
  Object.entries(obj).forEach(el => {
    keyValueArray.push(`<li>${el[0]}: ${el[1]}</li>`);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let notNumArray=input.map(ele=>ele.filter(ele=>!(ele%5)));
  let notStar=notNumArray.map(ele=>ele.filter(ele=>typeof(ele) !=='string'));
...
let findMaleAndFemale = (data) => {
  ...
  let arr=data.filter(el=>el.gender==='female' || el.gender==='male').map(el=>el.name).join(' and ');
  return arr;
};
